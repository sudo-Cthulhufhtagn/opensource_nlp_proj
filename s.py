...
LOGGER_REPO = os.getenv("LOGGER_REPO")
hf_writer = gr.HuggingFaceDatasetSaver(HF_TOKEN, LOGGER_REPO, private=True)
MODEL_ADDR = os.getenv("MODEL")
...
def classifier(text):
    ...
    hf_writer.flag([json.dumps(pred), {'text': text}], 'classify')
...
with gr.Blocks() as demo:
    ...
    dislike.click(lambda *args: hf_writer.flag([json.dumps(args[1]), {'text': args[0]}], "dislike"), [input_text, label], None, preprocess=False)
